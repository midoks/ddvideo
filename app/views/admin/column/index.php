...
layui.use(["table", "form", "laydate", "okLayer", "okUtils", "okMock"], function () {
	...
	let cTable = table.render({
		...
		cols: [[
			...
			{field: "type", title: "类型", width: 70,templet: "#typeTpl"},
	...
	function triggerStatus(id){
		...
		okUtils.ajax("/admincolumn/ajaxTriggerStatus", "get", {id: id}, true)
	...
	function deleteById(id) {
		okLayer.confirm("确定要删除吗？", function () {
			okUtils.isFrontendBackendSeparate = false;
			okUtils.ajax("/admincolumn/ajaxDelete", "get", {id: id}, true)
			.done(function (response) {
				okUtils.tableSuccessMsg(response.msg);
			}).fail(function (error) {
				// console.log(error);
	...
	function updateById(id) {
		okLayer.open("编辑", "/admincolumn/add.html?id=" + id, "60%", "70%", null, function () {
			cTable.reload();
		});
	}

	function add() {
		okLayer.open("添加", "/admincolumn/add.html", "60%", "80%", null, function () {
			cTable.reload();
		});
	}
});
